...
#elif _PCM_CH2_ROBOT_SOUND_02
#define AUDIO_OUT_SIZE     320
#define AUDIO_OUT_COUNT    2
#define AUDIO_BUF_SIZE     (AUDIO_OUT_SIZE*AUDIO_OUT_COUNT)
#else
#define AUDIO_OUT_SIZE     PCM_DEC_OUT_SIZE
#define AUDIO_OUT_COUNT    8
#define AUDIO_BUF_SIZE     (AUDIO_OUT_SIZE*AUDIO_OUT_COUNT)
#endif

#if _PCM_CH2_ECHO_SOUND
#define PCM_ECHO_DATA_LENGTH         40
#define PCM_ECHO_BUF_SIZE            PCM_ECHO_DATA_LENGTH*30
#endif
#if _PCM_CH2_ROBOT_SOUND_02
...
extern void CB_PCM_CH2_PlayStart(void);
extern void CB_PCM_CH2_PlayEnd(void);
extern void CB_PCM_CH2_RecStart(void);
extern void CB_PCM_CH2_RecEnd(void);
...
static void PCM_FreeMemory(U16 func)
{
    ...
    //}
#if _PCM_CH2_RECORD
    if(func&MEMORY_ALLOCATE_RECORD)   //AgcBuf
    {
#if AGC_ENALBE
        MemFree(AgcBuf);
        AgcBuf=NULL;
...
//          others:fail
//------------------------------------------------------------------// 
U8 PCM_CH2_Play(U32 index,U8 mode){
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetSpeed(S8 index){   
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetPitch(S8 index){   
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetRobotSound(U8 cmd,U8 index){
#if _PCM_CH2_ROBOT_SOUND   
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetRobot01Sound(U8 cmd,U8 type, U8 thres){
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetRobot02Sound(U8 cmd,U8 type, U8 thres){
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_SetReverbSound(U8 cmd,U8 index){
    ReverbCmd = cmd;
    if(cmd)
    {
#if _USE_HEAP_RAM
...
//          NONE
//------------------------------------------------------------------//
void PCM_CH2_SetEchoSound(U8 cmd,U8 index){
...
//------------------------------------------------------------------// 
#if _PCM_CH2_RECORD
U8 PCM_CH2_Record(U32 index,U8 mode,U16 sample_rate,U32 size){
    U8 i;
#if _USE_HEAP_RAM    
    PCM_AllocateMemory(MEMORY_ALLOCATE_PLAY|MEMORY_ALLOCATE_RECORD);
#endif    
    memset((void *)&AudioBufCtl, 0, sizeof(AudioBufCtl));
...
//          NONE
//------------------------------------------------------------------// 
void PCM_CH2_AdcIsr(U8 size,volatile U32 *pdest32){
    S16 *pdst16;
    U8 i;
    U16 temp;
...
//          NONE
//------------------------------------------------------------------//     
void PCM_CH2_ServiceLoop(void){
    
//    if((SmoothCtl.p_parameter&P_WAIT)&&(AudioBufCtl.state==STATUS_STOP)){
//        SmoothCtl.p_parameter&=(~P_WAIT);
//        PCM_Play(SmoothCtl.p_index,(SmoothCtl.p_parameter&P_MODE_MASK)>>P_MODE_SHIFT,
